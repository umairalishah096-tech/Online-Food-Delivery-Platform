...
const DISHES = [
  {
    ...
    image: "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=900&q=90",
  ...
  },
  {
    ...
    image: "https://images.unsplash.com/photo-1544025162-d76694265947?w=900&q=90",
    thumb: "https://images.unsplash.com/photo-1544025162-d76694265947?w=300&q=80",
    price: "$24.99",
    rating: 4.8,
    reviews: 1876,
    time: "35 min",
  ...
  },
  {
    ...
    image: "https://images.unsplash.com/photo-1540189549336-e6e99c3679fe?w=900&q=90",
...
export default function FoodShowcase() {
          ...
          <motion.div key="ingredients" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            {dish.ingredients.map((ing, i) => (
              <motion.div key={ing.name} initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }}
                ...
                className="flex items-center gap-3 bg-white/8 border border-white/10 rounded-xl px-3 py-2.5">
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f5]" style={{ fontFamily: "'Inter','Segoe UI',sans-serif" }}>
      ...
      <div className="fixed top-4 left-4 z-50">
        <Link to="/"
          className="flex items-center gap-1.5 text-white/80 hover:text-white text-sm font-medium
                     ...
                     transition-all hover:bg-white/22 shadow-lg">
          <ArrowLeft size={15} /> Home
        </Link>
      </div>

      {/* ── HERO ── */}
      <section ref={heroRef} className="relative h-screen min-h-[680px] overflow-hidden flex flex-col"
      ...
      {/* ── BOTTOM CAROUSEL ── */}
      <section className="bg-[#0f172a] sticky bottom-0 z-40 border-t border-white/5 shadow-2xl">
        <div className="max-w-5xl mx-auto px-4 py-4">
          <div ref={carouselRef} className="flex gap-3 overflow-x-auto scrollbar-hide">
            {DISHES.map((d, i) => (
              <motion.button key={d.id} data-thumb onClick={() => go(i)}
                whileHover={{ y: -3, scale: 1.02 }} whileTap={{ scale: 0.96 }}
                className={`flex-shrink-0 flex items-center gap-3 px-4 py-3 rounded-2xl border transition-all duration-300 cursor-pointer ${i === activeIdx ? "border-white/25 bg-white/10" : "border-white/8 bg-white/4 hover:bg-white/9 hover:border-white/15"}`}
                style={i === activeIdx ? { boxShadow: `0 0 0 1px ${d.accent}40, 0 4px 18px ${d.accent}20` } : {}}>
                <div className="relative flex-shrink-0">
                  <img src={d.thumb} alt={d.name}
                    className={`w-14 h-14 rounded-xl object-cover transition-all duration-300 ${i === activeIdx ? "opacity-100" : "opacity-55"}`}
                    style={i === activeIdx ? { boxShadow: `0 0 0 2px ${d.accent}` } : {}} />
                  {i === activeIdx && (
                    <motion.div layoutId="activeThumbDot"
                      className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-[#0f172a]"
                      style={{ backgroundColor: d.accent }} />
                  )}
                </div>
                <div className="text-left min-w-[108px]">
                  <p className={`text-sm font-bold transition-colors leading-tight ${i === activeIdx ? "text-white" : "text-white/50"}`}>{d.name}</p>
                  <p className="text-xs text-white/30 line-clamp-1 mt-0.5">{d.subtitle}</p>
                  <div className="flex items-center gap-1.5 mt-1">
                    <Star size={10} className="fill-amber-400 text-amber-400" />
                    <span className="text-xs text-white/50 font-medium">{d.rating}</span>
                    <span className="text-xs font-bold ml-0.5" style={{ color: d.accent }}>{d.price}</span>
                  </div>
                </div>
              </motion.button>
            ))}
      ...
      {/* ── MOBILE PANEL ── */}
      <section className="lg:hidden bg-[#111827] px-5 py-6 space-y-4">
        <AnimatePresence mode="wait">
          <motion.div key={`mob-${activeIdx}`}
            initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            transition={{ duration: 0.32 }} className="space-y-3">
            <div className="flex gap-1 bg-white/8 border border-white/10 rounded-xl p-1">
              {["overview", "ingredients"].map((t) => (
                <button key={t} onClick={() => setTab(t)}
                  className={`flex-1 py-2 text-sm font-semibold rounded-lg capitalize transition-all ${tab === t ? "bg-white text-gray-900 shadow" : "text-white/55 hover:text-white"}`}>
                  {t}
                </button>
              ))}
            </div>
      ...
      {/* ── ALL DISHES GRID ── */}
      <section className="bg-[#f0f2f5] py-16 px-5">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            <span className="text-xs font-bold uppercase tracking-widest text-orange-500">Our Showcase</span>
            <h2 className="text-3xl lg:text-4xl font-black text-gray-900 mt-2">Explore All Dishes</h2>
            <p className="text-gray-500 mt-2 max-w-sm mx-auto text-sm">Click any dish to load it in the showcase above.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {DISHES.map((d, i) => (
              <motion.div key={d.id}
                ...
                onClick={() => { go(i); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                className="cursor-pointer bg-white rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 group">
                <div className="relative h-44 overflow-hidden">
                  <img src={d.thumb} alt={d.name}
                    className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  <div className={`absolute inset-0 bg-gradient-to-t ${d.bg} opacity-40`} />
                  <div className="absolute top-3 left-3">
                    <span className="text-xs font-bold px-2 py-0.5 rounded-full text-white"
                      style={{ backgroundColor: `${d.accent}cc` }}>
                      {d.rank.split(" ")[0]}
                    </span>
                  </div>
                  {i === activeIdx && (
                    <div className="absolute inset-0 rounded-t-3xl border-2 pointer-events-none"
                      style={{ borderColor: d.accent }} />
                  )}
                </div>
                <div className="p-4">
                  <h3 className="font-bold text-gray-900 text-sm leading-tight">{d.name}</h3>
                  <p className="text-gray-400 text-xs mt-0.5 line-clamp-1">{d.subtitle}</p>
                  <div className="flex items-center justify-between mt-3">
                    <div className="flex items-center gap-1">
                      <Star size={11} className="fill-amber-400 text-amber-400" />
                      <span className="text-xs font-semibold text-gray-700">{d.rating}</span>
                    </div>
                    <span className="text-sm font-black text-gray-900">{d.price}</span>
                  </div>
                </div>
              </motion.div>
            ))}
